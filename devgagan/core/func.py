   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await message.reply_text("You are Banned. Contact -- @Zapadiya_milan_003")
            return 1
        ...
        return 1
         ...
         await message.reply_text("Something Went Wrong. Contact us @Zapadiya_milan_003...")
         return 1
...
async def progress_callback(current, total, progress_message):
    ...
    f"╰──────────────────╯\n\n"
    f"**__Powered by Team @SHELLBY_CHENNAL__**"
        )
